...
import numpy as np
import requests
from Database.db_connector import fetch_test_corpus, fetch_test_qrels
...
def log_evaluate(dataset_name, evaluate):
    if dataset_name == "A":
        log_path = 'Offline Documents/A/evaluate_logs.json'
    elif dataset_name == "Q":
        log_path = 'Offline Documents/Q/evaluate_logs.json'
    else:
        return
...
def evaluate_model(name, model_type="hybrid", k=10, min_rel_val=1):
    queries_dict = test_corpus[name]
    ...
    for query_id, query_text in queries_dict.items():
        ...
        # استخلاص المستندات ذات الصلة بناءً على min_rel_val
        relevance_dict = qrels_dict[query_id]
        relevant_docs = {doc_id for doc_id, rel in relevance_dict.items() if rel >= min_rel_val}
    ...
    log_evaluate(name, f"Evaluation Results on Dataset: {name}, Model: {model_type}, min_rel_val ≥ {min_rel_val}")
    log_evaluate(name, "-----------------------------------------------------")
    log_evaluate(name, f"Total Queries Evaluated: {total_queries}")
    log_evaluate(name, f"Mean Average Precision (MAP):     {np.mean(all_ap):.4f}")
    log_evaluate(name, f"Precision@{k}:                     {np.mean(all_precisions):.4f}")
    log_evaluate(name, f"Recall:                           {np.mean(all_recalls):.4f}")
    log_evaluate(name, f"Mean Reciprocal Rank (MRR):       {np.mean(all_rr):.4f}")
    log_evaluate(name, "-----------------------------------------------------")
...
def evaluate_api():
    ...
    try:
        evaluate_model(dataset, model_type=model_type, k=k, min_rel_val=min_rel_val)
